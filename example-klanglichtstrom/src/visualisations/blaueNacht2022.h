...
public:
    void setup(ofxDmx *dmx)
    {
        ...
        fboSettings.width = ofGetWidth()/3;
        fboSettings.height = ofGetHeight()/3*2;
    ...
    void update()
    {
        ...
        _fbo.begin();
        ofClear(0, 0, 0, 0);
        drawDimmer(0, _fbo.getWidth()/2, _fbo.getHeight()/5);
        drawDimmer(1, _fbo.getWidth()/2, _fbo.getHeight()/5*2);
        drawDimmer(2, _fbo.getWidth()/2, _fbo.getHeight()/5*3);
        drawDimmer(3, _fbo.getWidth()/2, _fbo.getHeight()/5*4);
    ...
    void drawDimmer(int index, float cx, float cy)
    {
        ...
        auto angle = glm::pi<float>() + glm::pi<float>()/6;
        auto length = 40;
        for(auto i = 0; i < 6; i++)
        {
